#!/usr/bin/env python3
"""
Field 审计 Part 3: 异质性场景 — Field 真正的价值在哪里？
"""
import json, math, random, numpy as np
from collections import defaultdict
from scipy.sparse import csr_matrix, diags, eye, lil_matrix
from scipy.sparse.linalg import cg


# ═══════════════════════════════════════════════════
# GMRF 引擎（精简版，只用 v2）
# ═══════════════════════════════════════════════════

class GMRF:
    def __init__(self, tree_file, edge_file):
        with open(tree_file) as f: data = json.load(f)
        nodes = data['nodes']
        kps = [n for n in nodes if n.get('level') == 'kp']
        self.n = len(kps)
        self.n2id = {nd['name']: nd['id'] for nd in kps}
        self.id2i = {kid: i for i, kid in enumerate([n['id'] for n in kps])}
        
        # Build adjacency
        adj = defaultdict(list)
        for nd in nodes:
            pid = nd.get('parent_id')
            if pid and nd['id'] in self.id2i and pid in self.id2i:
                adj[nd['id']].append((pid, 0.8)); adj[pid].append((nd['id'], 0.8))
        cbp = defaultdict(list)
        for nd in nodes:
            pid = nd.get('parent_id')
            if pid and nd['id'] in self.id2i: cbp[pid].append(nd['id'])
        for s in cbp.values():
            for i in range(len(s)):
                for j in range(i+1, len(s)):
                    adj[s[i]].append((s[j], 0.3)); adj[s[j]].append((s[i], 0.3))
        with open(edge_file) as f: llm = json.load(f)
        pair_seen = set()
        for e in llm:
            s = self.n2id.get(e.get('source_name', ''))
            t = self.n2id.get(e.get('target_name', ''))
            if not (s and t and s in self.id2i and t in self.id2i): continue
            si, ti = self.id2i[s], self.id2i[t]
            pair = tuple(sorted([si, ti]))
            if pair in pair_seen: continue
            pair_seen.add(pair)
            w = float(e.get('weight', 0.5))
            adj[s].append((t, w)); adj[t].append((s, w))
        row, col, dat = [], [], []
        for sk, ns in adj.items():
            if sk in self.id2i:
                si = self.id2i[sk]
                for tk, w in ns:
                    if tk in self.id2i:
                        row.append(si); col.append(self.id2i[tk]); dat.append(w)
        self.A = csr_matrix((dat, (row, col)), shape=(self.n, self.n))
        deg = np.array(self.A.sum(axis=1)).flatten(); deg[deg < 1e-8] = 1.0
        D_inv_sqrt = diags(1.0 / np.sqrt(deg), 0)
        self.L = eye(self.n, format='csr') - D_inv_sqrt.dot(self.A).dot(D_inv_sqrt)
    
    def diagnose(self, obs_idx, obs_val, obs_prec=None, lam=2.0, prior_mean=0.5, prior_prec=0.1):
        if obs_prec is None: obs_prec = [4.0] * len(obs_idx)
        Q_base = lam * self.L + prior_prec * eye(self.n, format='csr')
        d_data = np.zeros(self.n)
        for idx, prec in zip(obs_idx, obs_prec): d_data[idx] = prec
        D = diags(d_data, 0, shape=(self.n, self.n))
        b = Q_base.dot(np.full(self.n, prior_mean))
        for idx, val, prec in zip(obs_idx, obs_val, obs_prec):
            b[idx] += val * prec
        Q = Q_base + D
        mu, info = cg(Q, b, rtol=1e-6, maxiter=500)
        if info != 0:
            mu = np.full(self.n, prior_mean)
            for idx, val in zip(obs_idx, obs_val): mu[idx] = val
        return mu

    def build_topic_graph(self, tree_file):
        """返回基于 topic 分组的完美图"""
        with open(tree_file) as f: data = json.load(f)
        nodes = data['nodes']
        kps = [n for n in nodes if n.get('level') == 'kp']
        topics = defaultdict(list)
        for i, nd in enumerate(kps):
            pid = nd.get('parent_id', 'root')
            topics[pid].append(i)
        row, col, dat = [], [], []
        for pid, indices in topics.items():
            for i in range(len(indices)):
                for j in range(i+1, len(indices)):
                    row.extend([indices[i], indices[j]])
                    col.extend([indices[j], indices[i]])
                    dat.extend([1.0, 1.0])
        A_topic = csr_matrix((dat, (row, col)), shape=(self.n, self.n))
        deg_t = np.array(A_topic.sum(axis=1)).flatten(); deg_t[deg_t < 1e-8] = 1.0
        D_inv_sqrt_t = diags(1.0 / np.sqrt(deg_t), 0)
        return eye(self.n, format='csr') - D_inv_sqrt_t.dot(A_topic).dot(D_inv_sqrt_t)


# ═══════════════════════════════════════════════════
# 场景 1: 异质性学习 — 学生只学部分 Topic
# ═══════════════════════════════════════════════════

def simulate_heterogeneous(tree_file, n_students=200, n_days=100, seed=42):
    """
    学生随机选择 30% 的 topic 深入学习，其余浅尝辄止。
    这创造真正的 KP 间异质性。
    """
    with open(tree_file) as f: data = json.load(f)
    nodes = data['nodes']
    kps = [n for n in nodes if n.get('level') == 'kp']
    n = len(kps)
    
    np.random.seed(seed)
    # 按 parent_id 分组
    topic_difficulty = {}
    kp_to_topic = {}
    for nd in nodes:
        pid = nd.get('parent_id', 'root')
        if pid not in topic_difficulty:
            topic_difficulty[pid] = np.random.beta(2, 5)
    for i, nd in enumerate(kps):
        pid = nd.get('parent_id', 'root')
        kp_to_topic[i] = pid
    
    difficulty = np.zeros(n)
    for i, nd in enumerate(kps):
        pid = nd.get('parent_id', 'root')
        base = topic_difficulty.get(pid, 0.3)
        difficulty[i] = np.clip(base + np.random.normal(0, 0.05), 0.01, 0.99)
    
    # 学生 → 专注 topic
    all_topics = list(topic_difficulty.keys())
    results = []
    for s in range(n_students):
        np.random.seed(10000+s); random.seed(10000+s)
        # 每人随机选 30% topic 专注
        n_focus = max(1, int(len(all_topics) * 0.3))
        focus_topics = set(random.sample(all_topics, n_focus))
        
        K = np.zeros(n)
        observations = []
        
        for day in range(n_days):
            # 专注 topic 的 KP 获得更多复习
            focus_kps = [i for i in range(n) if kp_to_topic[i] in focus_topics]
            other_kps = [i for i in range(n) if kp_to_topic[i] not in focus_topics]
            
            # 80% 预算给专注 topic，20% 给其他
            budget = 10
            n_focus_review = max(1, int(budget * 0.8))
            n_other_review = budget - n_focus_review
            
            # Focus review
            urgencies = [(1.0-K[i])/max(K[i], 0.01) for i in focus_kps]
            top = sorted(focus_kps, key=lambda i: -urgencies[(focus_kps).index(i)])[:n_focus_review]
            for i in top:
                gamma = 0.30 * (1 - 0.6 * difficulty[i])
                K[i] += gamma * (1 - K[i])
            
            # Other review (fewer)
            urgencies_o = [(1.0-K[i])/max(K[i], 0.01) for i in other_kps]
            top_o = sorted(other_kps, key=lambda i: -urgencies_o[(other_kps).index(i)])[:n_other_review]
            for i in top_o:
                gamma = 0.30 * (1 - 0.6 * difficulty[i])
                K[i] += gamma * (1 - K[i])
            
            K *= 0.985; K += np.random.normal(0, 0.008, n); K = np.clip(K, 0, 1)
            
            if day > 0 and day % 14 == 0:
                n_test = max(3, int(n * 0.05))
                tested = random.sample(range(n), min(n_test, n))
                for i in tested:
                    p = 1.0/(1.0+math.exp(-5*(K[i]-difficulty[i])))
                    correct = random.random() < p
                    observations.append((day, i, correct))
                    if correct: K[i] += 0.03*(1-K[i])
                    K = np.clip(K, 0, 1)
        
        results.append({'true_K': K.copy(), 'observations': observations, 
                       'difficulty': difficulty.copy(), 'focus_topics': focus_topics,
                       'kp_to_topic': kp_to_topic.copy()})
    
    return results, difficulty, kp_to_topic


# ═══════════════════════════════════════════════════
# 场景 2: 前置依赖 — 天然异质性
# ═══════════════════════════════════════════════════

def simulate_prerequisite(tree_file, edge_file, n_students=200, n_days=100, seed=42):
    """
    学生必须先掌握前置 KP 才能学后续。
    Prerequisite 边创造结构化的 KP 差异。
    """
    with open(tree_file) as f: data = json.load(f)
    nodes = data['nodes']
    kps = [n for n in nodes if n.get('level') == 'kp']
    n = len(kps)
    n2id = {nd['name']: nd['id'] for nd in kps}
    id2i = {kid: i for i, kid in enumerate([nd['id'] for nd in kps])}
    
    # 读取 prerequisite 边
    with open(edge_file) as f: llm = json.load(f)
    prereqs = defaultdict(list); children = defaultdict(list)
    for e in llm:
        if e.get('edge_type') != 'prerequisite': continue
        s = n2id.get(e.get('source_name', ''))
        t = n2id.get(e.get('target_name', ''))
        if s and t and s in id2i and t in id2i:
            si, ti = id2i[s], id2i[t]
            prereqs[ti].append(si); children[si].append(ti)
    
    # 拓扑排序
    from collections import deque
    indeg = np.zeros(n)
    for i in prereqs: indeg[i] = len(prereqs[i])
    q = deque([i for i in range(n) if indeg[i] == 0])
    topo = []
    while q:
        i = q.popleft(); topo.append(i)
        for j in children[i]:
            indeg[j] -= 1
            if indeg[j] == 0: q.append(j)
    
    # 给无入度的 KP 赋予难度
    np.random.seed(seed)
    difficulty = np.random.beta(2, 5, n)
    
    results = []
    for s in range(n_students):
        np.random.seed(10000+s); random.seed(10000+s)
        K = np.zeros(n); learned = np.zeros(n, bool)
        observations = []
        
        for day in range(n_days):
            # 解锁新的 KP
            for i in range(n):
                if not learned[i]:
                    ok = all(learned[p] and K[p] > 0.3 for p in prereqs[i])
                    if ok: learned[i] = True; K[i] = 0.2
            
            # Urgency scheduling on learned KPs
            learned_list = [i for i in range(n) if learned[i]]
            if not learned_list: continue
            
            urgencies = [(1.0-K[i])/max(K[i], 0.01) for i in learned_list]
            top = sorted(learned_list, key=lambda i: 
                        -urgencies[learned_list.index(i)])[:min(10, len(learned_list))]
            
            for i in top:
                gamma = 0.30 * (1 - 0.6 * difficulty[i])
                K[i] += gamma * (1 - K[i])
            
            K *= 0.985; K += np.random.normal(0, 0.008, n); K = np.clip(K, 0, 1)
            
            if day > 0 and day % 14 == 0:
                testable = [i for i in range(n) if learned[i]]
                n_test = max(3, int(len(testable) * 0.05))
                tested = random.sample(testable, min(n_test, len(testable)))
                for i in tested:
                    p = 1.0/(1.0+math.exp(-5*(K[i]-difficulty[i])))
                    correct = random.random() < p
                    observations.append((day, i, correct))
                    if correct: K[i] += 0.03*(1-K[i])
                    K = np.clip(K, 0, 1)
        
        results.append({'true_K': K.copy(), 'observations': observations, 
                       'difficulty': difficulty.copy()})
    
    return results, difficulty


# ═══════════════════════════════════════════════════
# 评测
# ═══════════════════════════════════════════════════

def evaluate_scenario(sim_results, gmrf, gmrf_topic, n):
    """对比 Field vs baselines，用不同图"""
    methods = {
        'Naive': [],
        'Ability': [],
        'Field(λ=1)': [],
        'Field(λ=2)': [],
        'Field(λ=4)': [],
        'Field(topic graph,λ=1)': [],
        'Field(topic graph,λ=2)': [],
        'Field(no graph,λ=0)': [],
    }
    
    for r in sim_results:
        true_K = r['true_K']
        obs = r['observations']
        
        obs_dict = {}
        for day, idx, correct in obs:
            obs_dict[idx] = 0.85 if correct else 0.15
        obs_idx = list(obs_dict.keys())
        obs_val = [obs_dict[i] for i in obs_idx]
        
        # Naive
        mu = np.full(n, 0.5)
        for i, v in zip(obs_idx, obs_val): mu[i] = v
        methods['Naive'].append(np.mean(np.abs(mu - true_K)))
        
        # Ability
        avg = np.mean(obs_val) if obs_val else 0.5
        methods['Ability'].append(np.mean(np.abs(avg - true_K)))
        
        # Field variants
        for lam in [1, 2, 4]:
            mu = gmrf.diagnose(obs_idx, obs_val, lam=lam, prior_mean=0.5, prior_prec=0.1)
            methods[f'Field(λ={lam})'].append(np.mean(np.abs(mu - true_K)))
        
        # Topic graph
        for lam in [1, 2]:
            mu = gmrf_topic.diagnose(obs_idx, obs_val, lam=lam, prior_mean=0.5, prior_prec=0.1)
            methods[f'Field(topic graph,λ={lam})'].append(np.mean(np.abs(mu - true_K)))
        
        # No graph
        mu = gmrf.diagnose(obs_idx, obs_val, lam=0.01, prior_mean=0.5, prior_prec=0.5)
        methods['Field(no graph,λ=0)'].append(np.mean(np.abs(mu - true_K)))
    
    return {k: (np.mean(v), np.std(v)) for k, v in methods.items()}


def print_results(name, results):
    print(f"\n[{name}]")
    for method, (mae, std) in sorted(results.items(), key=lambda x: x[1][0]):
        print(f"  {method:30s}: {mae:.4f} ± {std:.4f}")


# ═══════════════════════════════════════════════════
# Main
# ═══════════════════════════════════════════════════

if __name__ == '__main__':
    TREES = {
        'CFA': ('cfa_tree.json', 'cfa_llm_edges.json'),
        'Math': ('math_tree.json', 'math_llm_edges.json'),
    }
    
    print("=" * 70)
    print("SCENARIO 1: 异质性学习 (30% topic focus)")
    print("=" * 70)
    
    for name, (tf, ef) in TREES.items():
        gmrf = GMRF(tf, ef)
        gmrf_topic = GMRF(tf, ef)
        gmrf_topic.L = gmrf.build_topic_graph(tf)
        
        het_results, difficulty, kp_to_topic = simulate_heterogeneous(tf)
        
        avg_obs = np.mean([len(r['observations']) for r in het_results])
        all_K = np.array([r['true_K'] for r in het_results])
        kp_means = all_K.mean(axis=0)
        
        print(f"\n{'─'*70}")
        print(f"[{name}] n={gmrf.n}  avg obs={avg_obs:.0f}  KP std={kp_means.std():.4f}")
        print(f"{'─'*70}")
        
        res = evaluate_scenario(het_results, gmrf, gmrf_topic, gmrf.n)
        print_results(name, res)
    
    print("\n\n" + "=" * 70)
    print("SCENARIO 2: 前置依赖学习")
    print("=" * 70)
    
    for name, (tf, ef) in TREES.items():
        gmrf = GMRF(tf, ef)
        gmrf_topic = GMRF(tf, ef)
        gmrf_topic.L = gmrf.build_topic_graph(tf)
        
        prereq_results, difficulty = simulate_prerequisite(tf, ef)
        
        avg_obs = np.mean([len(r['observations']) for r in prereq_results])
        all_K = np.array([r['true_K'] for r in prereq_results])
        kp_means = all_K.mean(axis=0)
        
        print(f"\n{'─'*70}")
        print(f"[{name}] n={gmrf.n}  avg obs={avg_obs:.0f}  KP std={kp_means.std():.4f}")
        print(f"{'─'*70}")
        
        res = evaluate_scenario(prereq_results, gmrf, gmrf_topic, gmrf.n)
        print_results(name, res)
