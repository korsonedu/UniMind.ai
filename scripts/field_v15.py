...
真实学习习惯的核心不是公式，是行为规则：
1. 已学 KP 中 K < DANGER 的超过 MAX_DEBT 个→停止学新，只复习
2. 复习债清到安全线以下→恢复学新
3. 前置必须达标才能学下游

对比 FSRS 和 Field，观察在自然学习节奏下的推进差距。
"""
...
DANGER_THRESHOLD = 0.30       # ★ K 低于此值→计入复习债
MAX_REVIEW_DEBT = 8           # ★ 复习债超过此数→暂停学新
EXAM_INTERVAL = 20
EXAM_COVERAGE = 0.3
BASE_DECAY = 0.005
DAY_NOISE = 0.01
...
print(f'n={n} roots={n_roots} max_chain={max_chain}')
...
def run_sim(sched, seed):
        ...
        if sched == 'field':
            ...
            for i in eligible:
                if not learned[i]:
                    ...
                    scores.append(NEW_ATTRACTION * (1.0 + BETA_A * fb))
                else:
    ...
    return {
        ...
        'max_depth': max_depth,
        'debt_stops': len(debt_stop_history),
    }
...
for sched in ['fsrs', 'field']:
    ...
    print(f"  debt_stops    = {agg['debt_stops'][0]:.1f}", flush=True)
