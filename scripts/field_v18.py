#!/usr/bin/env python3
"""
Field v18: 守门员 + 深度双重加权

v17 证明深度优先能推更深（chain_len +114%, n_deep 7 vs 0），但浅层崩塌了。
v18: 守门员加权保护前置链 + 深度加权推动深度推进。
"""
...
print(f'BUDGET={DAILY_BUDGET} DEPTH={DEPTH_BONUS} GK={GATEKEEPER_BONUS}')
        ...
        if sched == 'field':
            ...
            for i in eligible:
                if not learned[i]:
                    ...
                    base = urgency(u[i])
                    # 深度加权
                    base *= (1.0 + DEPTH_BONUS * depth_map[i])
                    # ★ 守门员加权（保护前置链）
                    if gatekeepers[i]:
...
for sched in ['fsrs', 'field']:
    ...
    print(f"  n_deep_mastered= {agg['n_deep_mastered'][0]:.0f}")
    print(f"  n_mastered     = {agg['n_mastered'][0]:.0f}")
