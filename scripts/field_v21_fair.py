#!/usr/bin/env python3
"""
Field v21 公平对比：FSRS-1.2 vs Field-Bellman
隔离 Bellman backup 的纯贡献（排除学新节奏差异）
"""
...
def bellman_values(learned,K,prereqs,dependents,topo,n):
    V=np.zeros(n)
    for i in range(n):
        if learned[i]: V[i]=urgency(K[i])
    for i in reversed(topo):
        for child in dependents[i]:
            V[i]+=ETA_VALUE*V[child]
    return V
...
def run_sim(sched, n, neighbors, prereqs, dependents, topo, seed):
    ...
    for day in range(N_DAYS):
        ...
        if use_bellman: V=bellman_values(learned,K,prereqs,dependents,topo,n)
        eligible=[]
        for i in range(n):
...
for name,(tf,ef) in TREES.items():
    ...
    nr=sum(1 for i in range(n) if not prereqs[i])
    nc=sum(1 for i in range(n) if dependents[i])
    print(f"\n{'='*60}\n[{name}] n={n} roots={nr} with_children={nc}")
