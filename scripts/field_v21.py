#!/usr/bin/env python3
"""
Field v21: Bellman Backup on Prerequisite DAG

核心：V(i) = urgency(K_i) + η × Σ_{子节点 j} V(j)
逆拓扑序一次计算。叶子 V = urgency，根 V = 整棵子树的总价值。

对比：
  FSRS:  score(i) = urgency(K_i)
  Field: score(i) = V(i)  (包含所有下游的价值)
"""
...
ALPHA   = 0.015
GAMMA   = 0.3
ETA_CASCADE = 0.05     # 物理级联
ETA_VALUE = 0.15       # ★ 价值传播折扣
N_STUDENTS = 500
N_DAYS = 150
DAILY_BUDGET = 8
INIT_K = 0.25
...
def load_graph(tree_file, edge_file):
    ...
    topo_list = []
    while q_dag:
        i = q_dag.popleft(); topo_list.append(i)
    ...
    return n, neighbors, prereqs, dependents, topo_list
...
def bellman_values(learned, K, prereqs, dependents, topo, n, eta_val):
    """
    ★ V(i) = urgency(K_i) + η × Σ_{子节点 j} V(j)
    逆拓扑序，从叶子往根传。

    对未学 KP：V(i) = potential_value(i)
    potential: 如果学了它，能解锁多少价值
    """
    V = np.zeros(n)
...
def run_sim(sched, n, neighbors, prereqs, dependents, topo, seed):
    ...
    for day in range(N_DAYS):
        ...
        # ★ Bellman values
        if use_bellman:
            V = bellman_values(learned, K, prereqs, dependents, topo, n, ETA_VALUE)
        ...
        if use_bellman:
            ...
            for i in eligible:
                if not learned[i]:
                    # 未学 KP：吸引力 + 下游潜在价值
                    scores.append(NEW_ATTRACTION + ETA_VALUE * V[i])
                else:
                    # 已学 KP：Bellman value
                    scores.append(V[i])
        else:
        ...
        if ll:
            cumul += float(np.mean([K[i] for i in ll]))
            cumul_days += 1
...
for name, (tf, ef) in TREES.items():
    ...
    n_children = sum(1 for i in range(n) if dependents[i])
    print(f"\n{'='*60}")
    print(f"[{name}] n={n} roots={n_roots} with_children={n_children}")
    ...
    for sched in ['fsrs', 'field']:
        ...
        print(f"  {sched:>6s}: mastered={agg['n_mastered']:.0f}  deep={agg['n_deep']:.0f}  "
              f"learned={agg['n_learned']:.0f}/{n}  final_K={agg['final_mastery']:.4f}",
              flush=True)
