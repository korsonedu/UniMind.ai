...
class InterviewAIService:
    ...
    def tune_resume(cls, resume_text: str):
        ...
        """
        default_prompt = (
            "你是一个资深的金融行业HR及面试官。请评估考生的简历，从排版、STAR法则、金融素养给出评分和润色后的内容。\n"
            "返回严格的 JSON 格式: \n"
            "{\n"
            '  "score": 85,\n'
    ...
    def generate_interview_reply(cls, session_type: str, style: str, chat_history: list):
        """
        基于 WebSocket 传来的上下文，生成面试官的下一句追问
        chat_history 格式: [{'role': 'interviewer', 'content': '...'}, {'role': 'candidate', 'content': '...'}]
        """
    ...
    def generate_post_interview_radar(cls, chat_history: list):
        ...
        """
        default_prompt = (
            "你是一个资深的金融考研复试分析专家。请根据这轮面试的完整对话记录，对考生进行客观、深度的五维复盘打分。\n"
    ...
    def annotate_candidate_turn(cls, session_type: str, answer_text: str):
        default_prompt = (
            "你是金融复试教研老师。针对考生刚刚回答的单句话，给出1到2句简短、毒辣的可执行改进建议。\n"
            "【规则】\n"
            "1. 禁止空泛的表扬。\n"
