...
from quizzes.models import (
    FSRSOptimizationLog, UserQuestionStatus, Question, QuizExam,
    PersonalizedMockExam, ReviewLog, FSRSProfile,
)
...
from quizzes.services.study_planner import calculate_study_plan, build_adaptive_question_ids
...
class StudyPlanView(APIView):
    ...
    def get(self, request):
        try:
            minutes = int(request.query_params.get('minutes', 25))
        except (ValueError, TypeError):
            minutes = 25
        try:
            preferred_limit = int(request.query_params.get('limit')) if request.query_params.get('limit') else None
        except Exception:
            preferred_limit = None

        plan = calculate_study_plan(
            user=request.user,
            minutes=minutes,
        ...
        if latest_exam:
            plan['latest_exam_summary'] = {
                'exam_id': latest_exam.id,
                'created_at': latest_exam.created_at,
                'summary': latest_exam.summary or '',
                'total_score': latest_exam.total_score,
                'max_score': latest_exam.max_score,
                'elo_change': latest_exam.elo_change,
            }
        adaptive_preview = build_adaptive_question_ids(
            user=request.user,
            limit=plan['recommended_questions'],
        )
        plan['next_session_mix'] = adaptive_preview['meta']
        return Response(plan)


class FSRSCurveView(APIView):
    permission_classes = [IsMember]
...
class FSRSOptimizationHistoryView(APIView):
    permission_classes = [IsMember]
...
class PersonalizedMockExamView(APIView):
    ...
    def post(self, request):
        # 先创建 processing 状态的记录，立即返回，任务异步执行
    ...
    def delete(self, request):
        ...
        # 删除关联的 PDF 文件（exam_pdf / answer_pdf 存储的是绝对路径字符串）
