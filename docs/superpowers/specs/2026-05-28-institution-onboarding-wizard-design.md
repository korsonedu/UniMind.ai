...
用户直接注册后（无机构），OnboardingDialog 首先展示角色选择：

| 选项 | 行为 |
|------|------|
| 我是学生 | 显示提示"请联系老师获取邀请链接"，提供返回按钮 |
| 我是教师/机构主 | 进入下方 5 步创建机构流程 |

学生选择后看到引导文案，无法进入系统，但账号保留。后续通过 `/join/:invite_slug` 邀请链接绑定机构。

### Step 2-6: 创建机构流程（教师路径）

| # | 卡片 | 输入类型 | 必填 | 说明 |
|---|------|---------|------|------|
| 2 | 邀请码 | 文本输入 | 是 | 验证 PlanInviteCode，通过后解锁后续卡片 |
| 3 | 机构名称 | 文本输入 | 是 | 对应 Institution.name |
| 4 | 业务规模 | 选项卡片 | 是 | 4 个预设选项：1-50 / 50-200 / 200-500 / 500+ |
| 5 | 主讲科目 | DirectionSelector | 是 | 复用现有组件，预设科目 + 自定义输入 |
| 6 | 机构简介 | 多行文本 | 否 | 可跳过，对应 Institution.description |

### 交互流程

1. 卡片 1 展示角色选择，选"教师/机构主" → 渐隐
2. 卡片 2 浮现，输入邀请码 → 点"下一步" → 验证通过 → 渐隐
3. 卡片 3 浮现 → 填名称 → 点"下一步" → 渐隐
4. 卡片 4 浮现 → 点选规模 → 立即渐隐
5. 卡片 5 浮现 → 选科目 → 点"下一步"
6. 卡片 6 浮现 → 填描述或点"跳过"
7. 调用 `POST /users/institutions/create/` 创建机构
8. 成功提示 → `window.location.reload()`

### 每张卡片引导文案

- 卡片 1: "你的身份是？"（学生/教师两个卡片选项）
- 卡片 2: "欢迎使用 UniMind！请输入您的邀请码开始创建机构"
- 卡片 3: "给你的机构起个名字吧"
- 卡片 3: "你的机构目前有多少学生？"
- 卡片 4: "你主要教哪些科目？"
- 卡片 5: "简单介绍一下你的机构（可选）"
