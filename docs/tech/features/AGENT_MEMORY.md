...
用户对话 → AIChatView
    │
    ├── 工具权限沙箱（filter_tools by plan）
    │
    ├── Prompt 组装
    │   ├── base system prompt
    │   ├── 机构人格（institution_personality）
...
规则引擎检测用户行为模式，生成自适应指令注入 system prompt。

| 组件 | 文件 | 说明 |
|------|------|------|
| Pattern Detector | `ai_assistant/services/prompt_adapter.py` | 8 种模式检测（关键词匹配） |
...
| Method | Path | 说明 |
|--------|------|------|
| GET | `/api/ai/memories/semantics/` | 语义记忆列表（支持 `?limit=N`） |
