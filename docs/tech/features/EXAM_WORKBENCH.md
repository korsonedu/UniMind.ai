...
工作台是老师/机构主的核心工作界面，采用**对话式 Agent 架构**——教师通过自然语言描述出题需求，Agent 自主调用工具搜索知识点、快速出题、ARC 精修、查询学生学情、发送通知。结构与小宇（XiaoYu）学习规划 Agent 对齐：未对话时大片留白，对话后左右分栏。

2026-05-31 重构：核心出题流程收敛为对话式指令驱动。2026-08-14 产品收敛：移除作业管理/班级运营/教学计划工具，收敛为 12 个工具（出题管线、题库、学生数据、通知）。
