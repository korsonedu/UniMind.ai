...
| 文件 | 改动 |
|------|------|
| `backend/ai_engine/service.py` | `call_ai_with_streaming_tools()` 流式 agent 循环 + `reasoning_content` 捕获 + `type:function` + DeepSeek 跳过 `tool_choice` |
...
- **DeepSeek 不支持 `tool_choice` 参数**：`service.py` 中两个 agent 方法均跳过（`if tool_choice is not None and not _is_deepseek`）。planner/exam_generator 的工具调用完全靠 system prompt 中的强制调用规则驱动。Free plan 的 planner 有 4 个基础工具（`get_learning_stats`/`get_knowledge_mastery_map`/`get_due_reviews`/`search_knowledge_tree`），定义在 `tool_permissions.py`
