...
const FEATURE_LABELS: Record<string, string> = {
  ...
  'basic.stats': '基础统计',
  'ai.generate': 'AI 生成题目',
  'memorix.review': 'Memorix 记忆复习',
  'full.report': '完整学情报告',
  'knowledge.graph': '交互式知识图谱',
  'course.video': '视频课程',
  'video.outline': 'AI 智能大纲',
  'faq.system': '在线答疑系统',
  'study.room': '实时自习室',
  'data.export': '数据导出',
  'brand.custom': '品牌定制',
  'api.access': 'API 接入',
  'student.payment': '学生端收费',
...
export default function InstitutionDashboard() {
  ...
  const FEATURE_KEYS = [
    ...
    'course.video', 'video.outline', 'faq.system',
    'study.room',
  ...
  if (data.mode === 'platform_admin' && data.institutions) {
    return (
      <div className="max-w-6xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-extrabold text-foreground tracking-tight">机构总览</h1>
          <p className="text-sm text-unimind-text-tertiary mt-1">{`共 ${data.institutions.length} 个机构`}</p>
        </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-extrabold text-foreground">{inst.name}
                      ...
                      {!inst.is_active && <Badge variant="outline" className="ml-1 text-[10px] text-red-500">已禁用</Badge>}
                    </p>
                    <p className="text-xs text-unimind-text-quaternary mt-0.5">
                      <Users className="h-3 w-3 inline mr-1" />{inst.student_count}/{inst.max_students} 学员数
                      <span className="mx-2">·</span>
                      <Calendar className="h-3 w-3 inline mr-1" />{inst.plan_expires_at?.slice(0, 10) || '永久有效'}
                    </p>
                  </div>
                </div>
              </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-sm text-unimind-text-tertiary">{inst.plan_expires_at?.slice(0, 10) || '永久有效'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: '学员数', value: `${inst.student_count} / ${inst.max_students}`,
            icon: Users, color: 'text-primary', bg: 'bg-primary/6' },
          { label: '管理员数', value: inst.admin_count, icon: GraduationCap,
            color: 'text-unimind-green', bg: 'bg-unimind-green/6' },
          { label: '周活跃学员', value: data.stats?.weekly_active_students ?? '—',
            icon: TrendUp, color: 'text-primary', bg: 'bg-primary/6' },
          { label: 'AI 出题用量', value: data.stats?.ai_usage?.limit
            ? `${data.stats.ai_usage.used}/${data.stats.ai_usage.limit}`
            : (data.stats?.ai_usage?.limit === null ? '不限' : '—'),
