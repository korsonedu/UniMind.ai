  ...
  return (
    <div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          { id: 'totalUsers', label: '用户总数', value: users.length, icon: Users, color: 'text-primary', bg: 'bg-primary/6' },
          { id: 'admins', label: '管理员', value: users.filter(u => u.role === 'admin').length, icon: Shield, color: 'text-amber-500', bg: 'bg-amber-500/6' },
          { id: 'superusers', label: '超级用户', value: users.filter(u => u.is_superuser).length, icon: Shield, color: 'text-purple-500', bg: 'bg-purple-50' },
          { id: 'students', label: '学员', value: users.filter(u => u.role === 'student').length, icon: GraduationCap, color: 'text-unimind-green', bg: 'bg-unimind-green/6' },
        ].map(s => (
    ...
    <Card variant="apple" className="p-5 space-y-4 h-[520px] flex flex-col">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="apple" size="sm" onClick={save} disabled={saving}><FloppyDisk className="h-3.5 w-3.5 mr-1" />保存</Button>
        </div>
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <p className="text-sm text-muted-foreground/60 mb-6">
        {institution.name} · {institution.plan_label} · 成员 {studentCount + teacherCount}/{institution.max_students}
      </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <Card variant="apple"
          ...
          <p className="text-[11px] font-bold text-muted-foreground">成员总数</p>
        </Card>
        <Card variant="apple"
          className={cn(
            'p-4 space-y-1 cursor-pointer transition-all duration-300 active:scale-[0.97]',
          ...
          <p className="text-[11px] font-bold text-muted-foreground">学员</p>
        </Card>
        <Card variant="apple"
          className={cn(
            'p-4 space-y-1 cursor-pointer transition-all duration-300 active:scale-[0.97]',
          ...
          <p className="text-[11px] font-bold text-muted-foreground">教师</p>
        </Card>
        <Card variant="apple" className="p-4 space-y-1">
          ...
          <p className="text-[11px] font-bold text-muted-foreground">平均 ELO</p>
        </Card>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap mb-4">
        <div className="flex items-center gap-1 bg-muted rounded-lg p-0.5">
          {(['all', 'student', 'teacher'] as const).map(tab => (
            <button key={tab} onClick={() => setRoleFilter(tab)}
              ...
              {{ all: '全部', student: '学员', teacher: '教师' }[tab]}
            </button>
          ))}
        </div>
        ...
        <select value={sortBy} onChange={e => setSortBy(e.target.value as typeof sortBy)}
          ...
          <option value="joined">最近加入</option>
          <option value="elo">ELO 高→低</option>
        </select>
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={cn('text-[10px]',
                        ...
                        {s.institution_role === 'teacher' || s.institution_role === 'owner' ? '教师' : '学员'}
                      </Badge>
                    </div>
                    <p className="text-xs text-muted-foreground">{s.email}{s.date_joined && <span className="text-muted-foreground/50"> · 加入于 {s.date_joined.slice(0, 10)}</span>}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                  ...
                  {(isOwner || user?.is_institution_admin) && s.id !== user?.id && s.institution_role !== 'teacher' && (
                    <Button variant="ghost" size="icon" className="h-8 w-8" aria-label={`Remove ${s.nickname || s.username}`}
                      onClick={async () => {
                        if (!(await confirm(`确认移除「${s.nickname || s.username}」？`))) return;
                  ...
                  {isOwner && s.id !== user?.id && s.institution_role === 'teacher' && (
                    <Button variant="ghost" size="icon" className="h-8 w-8" aria-label={`Remove teacher ${s.nickname || s.username}`}
                      onClick={async () => {
                        if (!(await confirm(`确认移除教师「${s.nickname || s.username}」？`))) return;
                        await api.delete(`/users/institution/me/students/${s.id}/`);
                        fetch();
                      }}>
                      <Trash className="h-3.5 w-3.5 text-muted-foreground hover:text-red-500" />
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { id: 'totalAnswered', label: '总答题', value: questions.total_answered, sub: `${questions.correct_rate}% 正确` },
          { id: 'mastered', label: '已掌握', value: questions.mastered, sub: `共 ${questions.total} 题` },
          { id: 'dueReview', label: '待复习', value: questions.due_review, sub: '记忆临界' },
          { id: 'weeklyStudy', label: '本周学习', value: activity.reviews_this_week },
        ].map(s => (
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-3 text-[10px] font-bold text-muted-foreground">
            {[
              ...
              { key: 'learning', label: '学习中', color: '#FF9500', count: mastery.learning },
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <textarea className="w-full h-40 rounded-xl border p-3 text-sm font-mono resize-none"
              placeholder='用户名,邮箱,密码,昵称' value={text} onChange={e => setText(e.target.value)} />
            {result && (
...
function ResetPasswordDialog({ userId, username }: { userId: number; username: string }) {
  ...
  return (
    <>
      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setOpen(true)} aria-label={`Reset password for ${username}`} title='确认重置'>
