...
import { ArrowLeft, GitMerge, Sparkle, X, MagnifyingGlassPlus, MagnifyingGlassMinus, ArrowsOut, CaretRight, CaretDown, MagnifyingGlass, Plus, Trash, Pencil } from '@phosphor-icons/react';
...
function TreeNodeView({
  ...
  return (
    <div>
      ...
      >
        {hasChildren && (
          open ? <CaretDown className="w-3 h-3 shrink-0 text-gray-300" /> : <CaretRight className="w-3 h-3 shrink-0 text-gray-300" />
        )}
                  ...
                  <div className="flex gap-1 pt-1">
                    {e.source_type !== 'tree' && (
                      <Button variant="ghost" size="sm" className="h-5 text-[10px] text-red-400 hover:text-red-600 px-1.5"
                        ...
                        <Trash className="w-2.5 h-2.5 mr-0.5" />删除
                      </Button>
                    )}
                  </div>
                </div>
      ...
      <div className="flex flex-col h-[calc(100vh-4rem)]">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b bg-white/80 shrink-0">
          ...
          <Button variant="outline" size="sm" onClick={() => {}}>
            <Sparkle className="w-3.5 h-3.5 mr-1.5" />LLM 分析
          </Button>
          <div className="flex items-center gap-0.5 bg-gray-100 rounded-lg p-0.5">
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setTransform(p => ({ ...p, k: Math.max(0.15, p.k / 1.3) }))}><MagnifyingGlassMinus className="w-3.5 h-3.5" /></Button>
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setTransform(p => ({ ...p, k: Math.min(3, p.k * 1.3) }))}><MagnifyingGlassPlus className="w-3.5 h-3.5" /></Button>
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setTransform({ x: 0, y: 0, k: 1 })}><ArrowsOut className="w-3.5 h-3.5" /></Button>
          </div>
        </div>
          ...
          <div className="w-56 border-r bg-white flex flex-col shrink-0">
            <div className="px-3 py-2 border-b">
              <div className="relative">
                <MagnifyingGlass className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-gray-300" />
